...
export default async function Index() {
  ...
  const grups = await supabase.from("Grups").select('*, dino_grup(*)').then(({ data: grups }) => {
  ...
  return <>
    ...
    <div className="flex flex-col gap-y-2 p-5 w-full">
      {grups && grups.map((grup, index) => <GroupCard key={index} grup={grup} position={index + 1} dino_grup={grup.dino_grup} />)}
    </div>
  </>
}